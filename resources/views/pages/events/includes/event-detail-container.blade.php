<div class="border rounded-4 p-custom-2 p-md-5 d-grid gap-3" style="max-width: 25rem">
    ...
    <ul class="d-grid gap-3 p-0 justify-content-center justify-content-md-start">
        ...
        <x-listitem.event-list-item icon="bi:people-fill">
            @if (count($event->registeredByUsers) < $event->eventDetail->quota)
                {{ count($event->registeredByUsers) ?? '-' }} / {{ $event->eventDetail->quota + count($event->registeredByUsers) ?? '-'}} Peserta
            @else
    ...
    <span class="d-flex justify-content-center align-items-center flex-column gap-3 w-full">
        ...
        @if (count($event->registeredByUsers) < $event->eventDetail->quota && date('Y-m-d') <= $event->eventDetail->max_register_date->format('Y-m-d'))
        ...
        @else
            <span class="d-inline-block" tabindex="0" data-bs-toggle="tooltip" data-bs-title="{{ count($event->registeredByUsers) >= $event->eventDetail->quota ? 'Kuota sudah penuh.' : 'Tanggal registrasi sudah lewat.' }} Silahkan pilih event lain!">
        ...
        @else
        <span class="d-inline-block" tabindex="0" data-bs-toggle="tooltip" data-bs-title="Anda tidak dapat melakukan cancel karena data Anda sudah terdata!">
            <button type="button" class="btn btn-sm border-0 rounded-pill py-2 btn-disabled pe-none" style="max-width: 18rem" disabled>
                Cancel
            </button>
        </span>
