<div class="border rounded-4 p-5 d-grid gap-3" style="max-width: 25rem">
    ...
    <span class="d-flex justify-content-center align-items-center flex-column gap-3 w-full">
        ...
        @can('register-event', $event)
            <button type="button" class="btn btn-sm btn-primary rounded-pill py-2" style="max-width: 18rem" data-bs-toggle="modal"
    ...
    <x-dialog.base-dialog id="registerEventModal"
        action="{{ route('event.action', ['actionType' => 'REGISTER_EVENT', 'slug' => $event->eventDetail->slug]) }}"
